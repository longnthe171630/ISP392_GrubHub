...
//                x = new Product(xProduct_id, xName, xPrice, xDescription, xImage, xStatus, xCreate_date, xCategory_id, xRestaurant_id);
    ...
    public List<Product> getProducts2() {
        List<Product> t = new ArrayList<>();
        xSql = "SELECT p.id, p.name, p.price, p.description, p.image, p.status, p.create_date,p.rating,p.quantity, "
    ...
    public List<Product> getProducTop9(int index) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                float xRating = rs.getFloat("rating");
                int xQuantity = rs.getInt("quantity");
                int s = rs.getInt("restaurant_id");
                int c = rs.getInt("category_id");
                x = new Product(xProductId, xName, xPrice, xQuantity, xDescription, xImage, xStatus, xRating, xCreate_date, s, c);
    ...
    public List<Product> getProductByRID(String cproduct_id) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                Restaurant s = rt.getRestaurantById(rs.getInt("restaurant_id"));
                Category c = cd.getCategoryId(rs.getInt("category_id"));
                Product x = new Product(xProductId, xName, xPrice, xQuantity, xDescription, xImage, xStatus, xPrice, xCreate_date, s, c);
                t.add(x);
            }
            rs.close();
            ps.close();
    ...
    public List<Product> getAllProducts() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                float rating = rs.getFloat("rating");
                Date dateAdded = rs.getDate("create_date");
                int categoryId = rs.getInt("category_id");
                int restaurantId = rs.getInt("restaurant_id");

                Product product = new Product(id, name, price, quantity, description, image, isActive, rating, dateAdded, categoryId, restaurantId);
    ...
    public int getTotalProductOfRestaurant(int restaurantId) {
        xSql = "select count(*) from product where restaurant_id = ?";
        try {
            ps = con.prepareStatement(xSql);
